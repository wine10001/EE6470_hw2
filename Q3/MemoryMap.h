...
public:
  MemoryMap(char const *the_memory_map_name,
    ...
    //read_counter = new int[_no_of_targets]();
    //write_counter = new int[_no_of_targets]();
  }
  ~MemoryMap() { 
    delete[] decodes; 
    //delete[] read_counter;
    //delete[] write_counter;
  }
  std::string memory_map_name() { return _memory_map_name; }

public:
  unsigned int no_of_targets() { return _no_of_targets; }
  unsigned int no_of_reads() { return _no_of_targets; }
  unsigned int no_of_writes() { return _no_of_targets; }

private:
  std::string _memory_map_name;
  const unsigned int _no_of_targets;
  //int* read_counter;  // Counter for read operations
  //int* write_counter; // Counter for write operations
