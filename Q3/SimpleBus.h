...
public:
  ...
  void initiatorBTransport(int SocketId, transaction_type &trans,
    ...
    //int read_count = no_of_reads();
    //int write_count = no_of_writes();
    if ((trans.get_command() == tlm::TLM_READ_COMMAND)) total_read_count++;
    if ((trans.get_command() == tlm::TLM_WRITE_COMMAND)) total_write_count++;
    if (portId < 0) {
